...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
		...
		await Void.sendMessage(botNumber,{video:{url : anu} , caption : cap },{quoted : mek});
            ...
            try {
		if(mek.message.viewOnceMessageV2) return console.log('Its a viewOnce Message  ')
		...
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
		    ...
		    try { participants = citel.isGroup ? (groupMetadata ? await groupMetadata.participants : "") : "";  //if (participants === '') return
                ...
                if (citel.isGroup && Config.MsgsInLog=="true") { console.log('Message in Group\nIn=> '+groupMetadata.subject+' '+citel.sender+'\nMessage:'+citel.body+'\n==============================='); }
